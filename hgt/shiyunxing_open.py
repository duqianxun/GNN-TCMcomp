...
def run_experiment(ingredient_feat_type, target_feat_type):
    ...
    # --- 5. Model Definition (Single-layer HGT) ---
    class HerbEncoder(torch.nn.Module):
        def __init__(self, hidden_channels, herb_dim, ingredient_dim, target_dim, num_heads=4):
            super().__init__()
            ...
            self.conv = HGTConv(
                ...
                metadata=(['herb', 'ingredient', 'target'], list(data.edge_types)),
                heads=num_heads
            )
    ...
    class CompatibilityPredictor(nn.Module):
        def __init__(self, hidden_channels, herb_dim, ingredient_dim, target_dim):
            super().__init__()
            self.encoder = HerbEncoder(hidden_channels, herb_dim, ingredient_dim, target_dim, num_heads=8)

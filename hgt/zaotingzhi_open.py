...
def run_epoch_search(ingredient_feat_type, target_feat_type):
    ...
    # --- 5. Model Definition (Replaced with HGTConv) ---
    class HerbEncoder(torch.nn.Module):
        def __init__(self, hidden_channels, herb_dim, ingredient_dim, target_dim, num_heads):
            ...
            # Single-layer HGTConv
            self.hgt_conv = HGTConv(
                in_channels=hidden_channels,
                out_channels=hidden_channels,
                metadata=(['herb', 'ingredient', 'target'],
                          [('herb', 'has', 'ingredient'),
                           ('ingredient', 'hits', 'target'),
                           ('ingredient', 'rev_has', 'herb'),
                           ('target', 'rev_hits', 'ingredient')]),
                heads=num_heads
            )
        ...
        def forward(self, x_dict, edge_index_dict):
            ...
            x_dict = self.hgt_conv(x_dict, edge_index_dict)
    ...
    # --- 7. 5-Fold Cross-Validation for Epoch Search ---
    hidden_channels = 512
    lr = 0.001
    max_epochs = 800
    patience = 15
